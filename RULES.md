...
```
${TF_BRANCH_PREFIX:+$TF_BRANCH_PREFIX/}{{task-name}}/{{agent-name}}
```

Examples:
- With `TF_BRANCH_PREFIX=user`: `user/fix-flaky-ci/red`
- With `TF_BRANCH_PREFIX=""` (empty): `fix-flaky-ci/red`

**Environment variable:** Optionally set `TF_BRANCH_PREFIX` to a prefix (e.g., your username).
